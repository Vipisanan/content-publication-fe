import axiosNotification from "./axios/axiosInstance";
// import axiosNotification from "./axios/axiosNotification";

// Get all subcriptions by user ID
export const fetchUserSubscriptions = (userId) =>
  axiosNotification.get(`/subscriptions/user/${userId}`);

// Subscribe to a category
export const subscribeToCategory = (userId, categoryId) =>
  axiosNotification.post(`/subscriptions/subscribe`, {
    userId,
    categoryId,
  });

// Unsubscribe from a category
export const unsubscribeFromCategory = (userId, categoryId) =>
  axiosNotification.post(`/subscriptions/unsubscribe`, {
    userId,
    categoryId,
  });

// Get all unread notifications for a user
export const fetchUnreadNotifications = (userId) =>
  axiosNotification.get(`/notifications/unread?userId=${userId}`);
